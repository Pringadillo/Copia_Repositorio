...
# ---------------------------------------- FUNCIONES DE VER COLUMNAS DE TABLA ----------------------------------------

def ver_tablas_BD(ruta_BD):
    """
    Muestra los nombres de todas las tablas en una base de datos SQLite específica.

    Args:
        ruta_BD (str): La ruta completa al archivo de la base de datos SQLite.
    """
    try:
        conn = sqlite3.connect(ruta_BD)
        cursor = conn.cursor()

        # Consulta para obtener los nombres de las tablas de sqlite_master
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table';")
        
        tablas = cursor.fetchall() # Obtiene todas las filas resultantes

        if tablas:
            print(f"\n--- Tablas en la base de datos: '{ruta_BD}' ---")
            for tabla in tablas:
                print(f"- {tabla[0]}") # tabla[0] es el nombre de la tabla
            print("------------------------------------------")
        else:
            print(f"\nNo se encontraron tablas en la base de datos: '{ruta_BD}'.")

    except sqlite3.Error as e:
        print(f"Error al conectar o leer la base de datos '{ruta_BD}': {e}")
    finally:
        if conn:
...
if __name__ == "__main__":
    ...
    #insertar_datos_iniciales_cuentas()
    #insertar_datos_iniciales_diario(ruta_BDapp)
